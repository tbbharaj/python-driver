...
class HostnameEndPointTest(unittest.TestCase):
    ...
    def test_multiple_endpoints_resolve(self):
        ...
        SniEndPoint._offset = -1  # reset the class attr
        endpoint = self.endpoint_factory.create_from_sni('test')
        endpoint2 = self.endpoint_factory.create_from_sni('test2')

        for _ in range(10):
            self.assertEqual(endpoint.resolve()[0], next(it))
            self.assertEqual(endpoint2.resolve()[0], next(it))
